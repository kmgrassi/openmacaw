...
> When work begins, copy to `local-runtime-helper/docs/` so the PR plan
  ...
  "tool_definitions": [
    {
      ...
      "metadata": { "bundle": "planner" }
    }
  ],
  "tool_calling_mode": "runtime_managed", // or "helper_managed"
  "provider_tool_specs": [ /* pre-translated for the provider */ ]
}
...
- `internal/agent/*` — wherever the helper currently has separate
  ...
  individual tool definition (looked up by tool name in the dispatch
  frame's `tool_definitions[]`).
