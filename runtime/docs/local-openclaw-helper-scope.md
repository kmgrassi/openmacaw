...
- **Persist to the existing `routing_rule.provider` column.** No new column
  ...
  **Reader/writer trace:**
  ...
  - **Reader** (PR 4 below — runtime change): Today, the execution-profile
    builder
    (`apps/orchestrator/lib/symphony_elixir/execution_profile.ex:430`)
...
- This satisfies the helper's "Advertise only initialized runners" rule:
  config parsing already accepts `[runner.openclaw]` and validates it, but
  today `cmdStart` only builds the `openai_compatible` runner. PR 5 closes
  the gap so the platform's PR 1 `[runner.openclaw]` TOML emission has an
  effect end-to-end.
