...
- `gateway_ws_opened`, `gateway_ws_closed`, `gateway_ws_frame_rejected`, `gateway_ws_upstream_failed`
...
### Browser Gateway Smoke

When testing the browser-to-runtime gateway path, capture the trace id from the
platform API/WebSocket request headers or from the first runtime
`gateway_ws_opened` event. Then search runtime logs for that exact
`trace_id` and confirm the same value appears on connection lifecycle,
frame rejection, upstream failure, request failure, and run events. A malformed
frame smoke should produce `gateway_ws_frame_rejected` with `connection_id`,
`workspace_id`, `agent_id`, `session_key`, `error_code`, and `retryable`
