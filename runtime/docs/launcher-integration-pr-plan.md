...
- [docs/architecture-launcher-integration.md](./architecture-launcher-integration.md) — target cross-repo architecture
- [apps/orchestrator/docs/worker_bridge_and_websocket_architecture.md](../apps/orchestrator/docs/worker_bridge_and_websocket_architecture.md) — how launcher `worker-bridge` and runtime `/ws` split
- [apps/orchestrator/docs/worker-bridge.md](../apps/orchestrator/docs/worker-bridge.md) — the shipped worker-bridge HTTP contract
- [apps/orchestrator/docs/runtime_websocket_gateway_contract.md](../apps/orchestrator/docs/runtime_websocket_gateway_contract.md) — the shipped `/ws` contract
- [apps/orchestrator/docs/db_agent_inventory.md](../apps/orchestrator/docs/db_agent_inventory.md) — how the launcher already reads Supabase `agent` and `credential`
- [supabase/generated/types.ts](../supabase/generated/types.ts) — live Supabase schema (source of truth for table shapes)

This document is organized by PR. Per-file task tracking lives in
[docs/implementation-tasks.md](./implementation-tasks.md).
...
|---|---|---|
| Multi-tenant boundary | [`workspaces`](../supabase/generated/types.ts#L3599) + [`workspace_members`](../supabase/generated/types.ts#L3563) + `is_workspace_member()` | Already the scope for agents, work_items, engine_instances. Don't create `projects`. |
| Durable agent identity | [`agent`](../supabase/generated/types.ts#L265) | Has `workspace_id`, `model_settings`, `tool_policy`, `status`, `current_version`. Created by the web client before the launcher starts anything. |
| Agent credentials | `credential` | Already resolved at launch time by `WorkerBridge.CredentialResolver`. |
| Running orchestrator process | [`engine_instance`](../supabase/generated/types.ts#L1260) | `instance_id`, `host`, `port`, `role`, `status`, `started_at`, `last_health_at`, `agent_id`, `workspace_id`, `ws_connection_id`. **This is what the launcher should write on start/stop.** |
| Launch config (tracker + runners) | [`gateway_config`](../supabase/generated/types.ts#L1400) + `gateway_config_versions` + `gateway_config_state` | Versioned, hashed, scoped config with sync status. Scoped by `(scope_type, scope_id)` — likely `('agent', <agent_id>)` or `('workspace', <workspace_id>)`. Confirm during OR-5. |
| Execution log (per-run) | [`broker_run`](../supabase/generated/types.ts#L761) | `agent_id`, `workspace_id`, `tracker_kind`, `tracker_issue_key`, `issue_identifier`, `status`, `attempt`, `input`, `output`, `codex_session_key`. **This is the persistent orchestrator-run log.** |
| Per-turn token usage | [`broker_task`](../supabase/generated/types.ts#L874) | Child of `broker_run`. `input_tokens`, `output_tokens`, `total_tokens`, `last_event`, `lease_expires_at`. |
...
|---|---|---|
| `POST /orchestrators` | `POST /agents/:id/start` | [db_agent_inventory.md:31](../apps/orchestrator/docs/db_agent_inventory.md#L31) |
| `GET /orchestrators` | `GET /agents` | [db_agent_inventory.md:23](../apps/orchestrator/docs/db_agent_inventory.md#L23) |
| `GET /orchestrators/:id` | `GET /agents/:id` | [db_agent_inventory.md:27](../apps/orchestrator/docs/db_agent_inventory.md#L27) |
| Session lifecycle | `POST/GET/DELETE /worker-bridge/sessions[/:id]` | [worker-bridge.md:46](../apps/orchestrator/docs/worker-bridge.md#L46) |
| Runtime chat transport | `GET /ws` (per-orchestrator) | [runtime_websocket_gateway_contract.md:33](../apps/orchestrator/docs/runtime_websocket_gateway_contract.md#L33) |

The doc at [architecture-launcher-integration.md](./architecture-launcher-integration.md) still
...
- ~~Orchestrator does not persist execution logs to `broker_run` / `broker_task`. See OR-6.~~ Shipped: `SymphonyElixir.BrokerLog` + `AgentRunner` lifecycle hook + boot-time orphan reconcile.
...
No items from [architecture §What the other repo needs to build](./architecture-launcher-integration.md#what-the-other-repo-needs-to-build) exist yet. Enumerated as PL-* below.
...
- [ ] Update `Tracker` behavior typespecs ([tracker.ex:8-12](../apps/orchestrator/lib/symphony_elixir/tracker.ex#L8)) from `[term()]` to `[WorkItem.t()]`
...
- [ ] Remove the wildcard fallback at [tracker.ex:46](../apps/orchestrator/lib/symphony_elixir/tracker.ex#L46); unknown `tracker.kind` must raise
...
metadata under `config["stored_agent"]` ([db_agent_inventory.md:47](../apps/orchestrator/docs/db_agent_inventory.md#L47)).
...
- [x] Add `SymphonyElixir.Launcher.GatewayConfig` module: `fetch/2` by `(scope_type, scope_id)`, `record_apply_state/4` writing `gateway_config_state`.
- [x] On `POST /agents/:id/start`:
...
- [ ] Config hot-reload: on a `gateway_config` version bump (polled or pushed — decide), restart the orchestrator or reload config in place, and update `gateway_config_state` again. *(deferred — follow-up; this PR lands the resolution + apply-state contract first.)*
- [x] Integration test: seed `gateway_config` for an agent; call `POST /agents/:id/start`; assert orchestrator runs with the seeded tracker/runners and that `gateway_config_state` reflects the applied version.
...
- [x] `SymphonyElixir.BrokerLog` module wrapping Supabase writes; no-op when Supabase env vars absent.
- [x] Hook into `AgentRunner` lifecycle (start, per-turn, end).
- [x] Token accounting already exists ([token_accounting.md](../apps/orchestrator/docs/token_accounting.md)) — wire it to `broker_task` row inserts.
- [x] Reconcile on orchestrator startup: any `broker_run` rows in `status='started'` for this agent that aren't actually running → mark `failed` with `terminal_reason='orphaned'`. (Keyed on `agent_id` from `stored_agent` — `broker_run` has no `engine_instance` FK today; adding one is out of scope for OR-6.)
- [x] Tests: mock Supabase; assert broker_run insert, broker_task per-turn, finish_run patch, and orphan reconcile.
...
- [ ] Update [WORKFLOW.md](../WORKFLOW.md) snippets
...
- [ ] Replace `POST /orchestrators` with `POST /agents/:id/start` in [architecture-launcher-integration.md](./architecture-launcher-integration.md)
...
- [ ] Typed request/response models matching the shipped contracts (see [worker-bridge.md:52](../apps/orchestrator/docs/worker-bridge.md#L52))
...
through a platform-owned proxy. See [worker_bridge_and_websocket_architecture.md:90](../apps/orchestrator/docs/worker_bridge_and_websocket_architecture.md#L90).
