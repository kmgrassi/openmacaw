...
3. **Generalizes beyond GitHub for free.** The manager loop
   ...
   *[Per-vertical generalization](#per-vertical-generalization)*
   below.
...
and emits escalations through the same `escalate_to_human` tool
as any author or reviewer ([OQ-06](./oq-06-escalation-policy-schema.md)).
...
- Speed of iteration is real but doesn't justify replacing Git for
  ...
  *fewer* PRs than today; it's *more* PRs that need a mechanism for
...
5. **Peer review fires automatically** when:
   ...
   - Workspace `gateway_config.body.gates.policies_by_kind.code`
     lists `peer_review` as a required gate (it does by default
     — see [OQ-07](./oq-07-auto-merge-gate-selection.md)).
   - A *separate* runner is dispatched (different model where
...
6. **External reviews** (a human teammate's comments) loop back via
   ...
   - `issue_comment.created` (when the comment matches a
     configured `@<bot-name>` mention — see *[@-mention triggers](#mention-triggers)*
...
3. Review of non-code outputs happens **in our dashboard** (we
   ...
   provide.
4. For text-and-markdown work that *is* git-friendly (research
...
- The agent considers the task "done from its perspective" when it
  has a green local lint+tests run and a self_review pass.
- It opens the PR. Opening the PR is what fires peer_review (via
  the gate runner) and external CI (via the existing GitHub
  Actions workflow).
- Review feedback (whether from the peer_review agent or a human)
...
*"Here's a list of due tasks. For each one, read the current
state and decide the smallest next action."* Only the
...
3. Match found → bump `work_item.next_poll_at = now()`. The
   ...
   and calls `dispatch_runner(runner_kind: <matched>, intent:
   <matched>, context: <comment body + PR>)`.
...
2. **Cross-model peer review on demand.** PR auto-passed our
   default peer_review (model A reviewing model A's diff). A
...
involves LLMs, and even then in a *bounded* way — one manager
...
- **Schedule.** The manager runs on a clock. Default: each
  ...
  `poll_cadence_seconds` controls how often a *non-bumped*
...
- **Toolset.** The manager has *meta-level* tools that act on
  tasks, not within them. See below.

#### The manager's tools

Same function-calling pattern as `create_plan`
([OQ-01](./oq-01-plan-format.md)) and `escalate_to_human`
([OQ-06](./oq-06-escalation-policy-schema.md)) — typed
function-call schema, no text-to-JSON parsing.
...
| `dispatch_runner(work_item_id, runner_kind, intent, context)` | Spawns an author / reviewer / mentioned-bot turn with the given intent. |
...
| `post_comment(work_item_id, body)` | Posts a comment on the artifact (PR comment, Figma annotation, etc.). |
| `escalate_to_human(work_item_id, …)` | Same tool from [OQ-06](./oq-06-escalation-policy-schema.md). Manager-emitted escalations capture what *the manager* observed, not the author runner. |
| `snooze(work_item_id, seconds)` | "Nothing to do; check again in N seconds." Updates `next_poll_at`. |
| `mark_done(work_item_id)`     | Closes the task as completed (work delivered, PR merged, video shipped). |
...
```jsonc
{
  ...
  "tools": [
    "read_artifact_state", "dispatch_runner", "merge_pr",
    "post_comment", "escalate_to_human", "snooze", "mark_done"
  ]
}
```

Note that the manager doesn't see the *content* of every event
...
- **Idempotent.** Running the same tick twice on the same state
  ...
  merged PR; `dispatch_runner` for an intent already in flight
  short-circuits).
...
manager dispatches them via `dispatch_runner` with an intent
...
| `check_run.completed` (red)    | bump `next_poll_at`  | "tests failed for PR #142"           | `dispatch_runner(intent: 'fix_tests')` or escalate |
| `pull_request_review.submitted`, `request_changes` | bump | "review requesting changes"          | `dispatch_runner(intent: 'address_review')` |
| `pull_request_review.submitted`, `approve` | bump | "review approved"            | `merge_pr` if other gates green |
| `pull_request_review_comment` (action `created`) | bump (debounced 30s) | "N comments since last poll"   | one `dispatch_runner` covering all comments |
| `@-mention` matched in `mention_handler` | bump (immediate) | "user pinged @harper-claude for review" | `dispatch_runner(runner_kind: <matched>, intent: <matched>)` |
...
manager reads artifact state + recent events for this work_item:
   "PR open, peer_review needed, no CI yet"
   ↓
manager calls dispatch_runner(intent: 'peer_review')
   ↓
...
manager next tick:
   "review approve, gates green" → calls merge_pr
                                  OR
   "review request_changes"      → calls dispatch_runner(intent: 'address_review')
                                  OR
   "stuck after K polls"          → calls escalate_to_human
   ↓
PR merged → manager calls mark_done → work_item.state = done
```
...
events. The manager reads the *current state* of the artifact
...
- Git is the slowest part of *nothing* in our stack. Optimizing it
...
5. **Manager toolset.** `read_artifact_state`,
   `read_recent_events`, `dispatch_runner`, `merge_pr`,
   `post_comment`, `escalate_to_human`, `snooze`, `mark_done`.
...
11. **Stuck-task detection.** Manager flags `escalate_to_human`
...
- For very long-running PRs (peer_review agent had a slow turn),
