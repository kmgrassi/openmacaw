...
> planning. A mirror copy is committed in
> `parallel-agent-platform/docs/local-model-readiness-platform-prs.md`
> on branch `claude/local-model-readiness-scope` so the implementation
...
| `apps/api/src/services/agent-runtime-profile.ts` (new) | Resolves and persists the per-agent runtime profile. For planner: `runners.planner.<agent_id>.{provider, model, credential_id}`. For coding: `runners.local_model_coding.<agent_id>.{...}` (the runtime keys the coding runner as `local_model_coding`, not `coding`). Validates `provider` against the runtime allowlist (`openai`, `openai_compatible`, `local`, `anthropic`, etc. — pull from runtime `execution_profile.ex`). |
