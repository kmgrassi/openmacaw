...
> **Mirrored.** This same design doc also lives in the platform repo at
> [`docs/auth-jwt-design.md`](https://github.com/kmgrassi/parallel-agent-platform/blob/main/docs/auth-jwt-design.md).
...
**Confirmed safe for the current deployment**, with two caveats.

**Confirmed in `apps/orchestrator/deploy/terraform/main.tf`:**
- `alb_internet_facing: false` in both staging and prod tfvars — the ALB
  lives inside the VPC, not on the open internet.
- `assign_public_ip = false` on the ECS task (`main.tf:391`).
- The service security group only accepts ingress from the ALB security
  group (`main.tf:191-197`). No direct internet path to the container.
- Task subnets resolve to the supplied `private_subnets` when provided;
  both staging and prod pass private subnets explicitly.
- Only `container_port: 4000` (orchestrator HTTP / `/ws`) is in the ALB
  target group. Port `4100` (launcher) is not exposed.

**Caveats:**

1. **The launcher control plane (`:4100`) isn't deployed in Terraform yet
   at all.** The container command runs the `symphony` escript starting a
   single orchestrator on `:4000`, not `mix launcher.start`. When the
   launcher lands in prod, the deployment must not add `:4100` to any
   portMapping or ALB target group. Bandit now binds to `127.0.0.1` by
   default (see `launcher/supervisor.ex`) so a misconfiguration
   (`0.0.0.0` bind) alone can't expose it — override with
   `LAUNCHER_BIND_HOST` only when deliberately doing so.

2. **The internet-facing path lives in a separate Terraform module.** The
   orchestrator stack reads `shared_platform_state` outputs (`vpc_id`,
   `https_listener_arn`, `alb_security_group_id`). That stack lives outside
   this repo — confirming no stray listener rule forwards public traffic
   to the orchestrator target group still needs a manual review of the
   shared platform Terraform. Low risk given the internal-ALB constraint,
   but worth a checklist item before the auth middleware ships.

If those caveats ever fail to hold (e.g. the runtime is intentionally
exposed to a partner), fall back to **Option A**: port Harper's
JWT-verification pattern to Elixir (`joken` + JWKS) and verify on the
runtime too. Option B is correct today; we're not locked out of A.
...
- [ ] Extend
      [`gateway_socket.ex` `scope_from_query/1`](apps/orchestrator/lib/symphony_elixir_web/gateway_socket.ex#L36)
      to read `user_id` from query params. Reject the connect with a
      `runtime_scope_required` error if absent.
- [ ] Widen
      [`SessionStore`](apps/orchestrator/lib/symphony_elixir/gateway/session_store.ex#L8)
...
- [ ] Update
      [`runtime_websocket_gateway_contract.md`](apps/orchestrator/docs/runtime_websocket_gateway_contract.md)
