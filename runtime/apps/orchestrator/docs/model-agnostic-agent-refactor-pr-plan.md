...
Runtime companion to the platform scoping document for making every agent role
model/provider agnostic: planning, coding, manager, and future custom agents.
...
- credentials are referenced by ID or alias, never embedded in labels or
  prompts;
...
Runtime has `SymphonyElixir.Runner.Planner`, but it is OpenAI
Responses-specific:
...
- Planning Agent + Anthropic Claude:
  ...
  - provider: `anthropic`
  - model: `claude-sonnet-...`
  - tool_profile: planner_safe read/create-plan tools
...
5. **Credentials never enter prompts or persisted raw events.**
   Credentials resolve by ID or alias at dispatch time, live in memory briefly,
   and are redacted from logs, events, and prompts.
...
Platform should send or make available a resolved profile in this shape:
...
Runtime should translate that into provider adapter execution:
...
- Add the shared `ExecutionProfile` contract under `contracts/`.
- Add provider/runner enums that include OpenAI, Anthropic,
  OpenAI-compatible, Codex, and OpenClaw without treating them as the same
  concept.
...
- Return explicit missing requirement reasons.
...
- Local/manual smoke path for Planning Agent on provider A creating a plan, user
  approving selected tasks, and Coding Agent on provider B receiving the
  handoff.
