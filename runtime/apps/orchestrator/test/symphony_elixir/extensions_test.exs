defmodule SymphonyElixir.ExtensionsTest do
  ...
  import SymphonyElixir.TestSupport, only: [put_system_env: 2, write_workflow_file!: 1, write_workflow_file!: 2]
  ...
  test "workflow current prefers a launcher-registered instance workflow over the global workflow store" do
    ensure_workflow_store_running()
    {:ok, registry_pid} = start_supervised(SymphonyElixir.Launcher.ConfigRegistry)

    launcher_workflow =
      Path.join(Path.dirname(Workflow.workflow_file_path()), "LAUNCHER_WORKFLOW.md")

    write_workflow_file!(launcher_workflow, tracker_kind: "api", prompt: "Launcher prompt")
    SymphonyElixir.Launcher.ConfigRegistry.put(self(), launcher_workflow)

    on_exit(fn ->
      SymphonyElixir.Launcher.ConfigRegistry.delete(self())
      if Process.alive?(registry_pid), do: GenServer.stop(registry_pid)
      File.rm_rf(launcher_workflow)
    end)

    assert {:ok, %{prompt: "Launcher prompt", config: %{"tracker" => %{"kind" => "api"}}}} =
             Workflow.current()
  end
  ...
  test "tracker delegates to memory and linear adapters" do
    issue = %WorkItem{id: "issue-1", identifier: "MT-1", state: "In Progress"}
  ...
  test "phoenix observability api preserves state, issue, and refresh responses" do
    ...
    root_code_access = %{
      "type" => "workspace",
      "label" => Path.basename(workspace_root),
    ...
    assert state_payload == %{
             ...
             "repo_cache" => %{"repositories" => [], "active_workspaces" => []},
    ...
    assert issue_payload == %{
             ...
             "status" => "running",
             "workspace" => %{
               "path" => issue_workspace_path,
               "host" => nil,
               "code_access" => %{
                 "type" => "workspace",
                 "label" => Path.basename(issue_workspace_path),
                 "value" => issue_workspace_path,
  ...
  test "phoenix observability api exposes local runtime diagnostics before issue routes" do
    ...
    start_test_endpoint(
      orchestrator: Module.concat(__MODULE__, :LocalRuntimeDiagnosticsOrchestrator),
    ...
    assert [%{"machine_id" => "machine-local", "runners" => [runner]}] = payload["helpers"]
