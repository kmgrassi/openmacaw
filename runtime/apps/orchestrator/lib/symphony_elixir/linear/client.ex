defmodule SymphonyElixir.Linear.Client do
  ...
  mutation SymphonyPlannerLinearIssueCreate($input: IssueCreateInput!) {
  ...
  mutation SymphonyPlannerLinearIssueUpdate($id: String!, $input: IssueUpdateInput!) {
  ...
  def graphql(query, variables \\ %{}, opts \\ [])
    ...
    request_fun =
      Keyword.get(opts, :request_fun, fn payload, headers ->
        post_graphql_request(payload, headers, opts)
      end)
  ...
  def create_issue(input, opts \\ []) when is_map(input) and is_list(opts) do
    with {:ok, body} <-
           graphql(@issue_create_mutation, %{"input" => input}, Keyword.put(opts, :operation_name, "SymphonyPlannerLinearIssueCreate")) do
      decode_mutation_result(body, ["issueCreate"], :linear_issue_create_failed)
    end
  end

  @spec update_issue(String.t(), map(), keyword()) :: {:ok, map()} | {:error, term()}
  def update_issue(issue_id, input, opts \\ [])
      when is_binary(issue_id) and is_map(input) and is_list(opts) do
    with {:ok, body} <-
           graphql(
             @issue_update_mutation,
             %{"id" => issue_id, "input" => input},
             Keyword.put(opts, :operation_name, "SymphonyPlannerLinearIssueUpdate")
           ) do
      decode_mutation_result(body, ["issueUpdate"], :linear_issue_update_failed)
    end
  end

  @doc false
  @spec normalize_issue_for_test(map()) :: WorkItem.t() | nil
  def normalize_issue_for_test(issue) when is_map(issue) do
    normalize_issue(issue, nil)
  end

  @doc false
  @spec normalize_issue_for_test(map(), String.t() | nil) :: WorkItem.t() | nil
  ...
  @doc false
  @spec merge_issue_pages_for_test([[WorkItem.t()]]) :: [WorkItem.t()]
  ...
  defp sort_issues_by_requested_ids(issues, issue_order_index)
    ...
    Enum.sort_by(issues, fn
      %WorkItem{id: issue_id} -> Map.get(issue_order_index, issue_id, fallback_index)
      _ -> fallback_index
    end)
  end
  ...
  defp decode_mutation_result(%{"data" => data}, path, failure_reason) when is_map(data) do
    case get_in(data, path) do
      %{"success" => true, "issue" => issue} when is_map(issue) ->
        {:ok, issue}

      %{"success" => false} = payload ->
        {:error, {failure_reason, payload}}

      payload ->
        {:error, {:linear_unknown_mutation_payload, payload}}
    end
  end

  defp decode_mutation_result(%{"errors" => errors}, _path, _failure_reason) do
    {:error, {:linear_graphql_errors, errors}}
  end

  defp decode_mutation_result(payload, _path, _failure_reason) do
    {:error, {:linear_unknown_mutation_payload, payload}}
  end

  defp post_graphql_request(payload, headers, opts) do
    endpoint = Keyword.get(opts, :endpoint) || Config.settings!().tracker.endpoint
  ...
  defp parse_priority(priority) when is_integer(priority), do: Integer.to_string(priority)
  defp parse_priority(priority) when is_binary(priority), do: priority
