defmodule SymphonyElixir.LogFile do
  ...
  defp setup_disk_handler(log_file, max_bytes, max_files) do
    ...
    case File.mkdir_p(Path.dirname(expanded_path)) do
      :ok ->
        add_disk_handler(expanded_path, max_bytes, max_files)

      {:error, reason} ->
        Logger.warning(
          "Failed to create rotating log directory #{Path.dirname(expanded_path)}: #{inspect(reason)}"
        )
  ...
  defp add_disk_handler(expanded_path, max_bytes, max_files) do
