...
- `Tracker.Database` already writes status back to the canonical `task` table
  through `work_items.task_id`.
- `WorkItem` already includes `task_id` and `plan_id`.

Platform:

- Supabase has `agent.type`, `agent.model_settings`, and `agent.tool_policy`.
- Supabase has canonical `plan` and `task` tables.
...
proves the existing `plan`, `task`, `agent`, `gateway_config`, and
...
- `task.create`
  - input: `workspace_id`, optional `plan_id`, `name`, optional `description`,
    optional `priority`, optional `labels`, optional `metadata`
...
- `task.update`
  - input: `workspace_id`, `task_id`, allowed fields: `name`, `description`,
    `priority`, `labels`, `metadata`, `status`
  - `task_id` must be the database `task.id` UUID
...
Goal: define the planner tool interface and enforce planner-safe tool policy.
...
- A planning agent receives only planner-safe tools by default.
