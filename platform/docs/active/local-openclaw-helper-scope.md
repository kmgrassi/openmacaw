...
- **Persist to the existing `routing_rule.provider` column.** No new column
  or migration needed — that column's check constraint already permits the
  runtime-family identifiers (`openclaw`, `codex`, `computer_use`, `local`)
  alongside the LLM-provider ids. See
  `harper-server/supabase/migrations/20260513150000_expand_routing_rule_provider_check.sql:42-45`,
  which comments the column explicitly as "Runtime-family providers (no
  per-row credential; the runner itself knows how to authenticate /
  dispatch)."

  **Reader/writer trace:**
  - **Writer** (this PR): UI binds the dropdown's selected value to the
    request body's `provider` field; the API route in
    `apps/api/src/routes/routing-rules.ts` already passes `provider`
    straight through to the DB insert/update.
  - **Reader** (PR 4 below — runtime change): Today, the execution-profile
    builder
    (`parallel-agent-runtime/apps/orchestrator/lib/symphony_elixir/execution_profile.ex:430`)
    sets `default_provider_for_runner("local_relay") = "local"` and never
    forwards the `provider` value to `Runner.LocalRelay`'s
    `target_runner_kind` config. PR 4 changes that path so that, for a
    `local_relay` rule, `target_runner_kind` is taken from
    `routing_rule.provider` (falling back to the existing default if
    unset). `Runner.LocalRelay` already keys its registry lookup on
    `target_runner_kind` (`runner/local_relay.ex:18, 70`), so the
    downstream dispatch path is unchanged.

### PR 4 — `parallel-agent-runtime`: thread `provider` → `target_runner_kind` + integration test

- **Code:** Update
  `apps/orchestrator/lib/symphony_elixir/execution_profile.ex` so that, when
  building a config for `runner_kind = local_relay`, `target_runner_kind`
  is sourced from the routing rule's `provider` column (falling back to
  `@default_target_runner_kind = "openai_compatible"` when unset). This is
  the reader side of the writer trace in PR 3.
- **Test:** Integration test covering registration + dispatch with the new
  mapping. Helper registers `runner_kinds: ["openclaw"]`; a routing rule
  with `runner_kind = local_relay, provider = openclaw` dispatches a
  request via `Runner.LocalRelay`; the dispatch frame lands on that
  helper (verified via `LocalRelay.Registry.lookup`).
- Confirm `local_relay_socket.ex` already accepts `openclaw` in
  `RegisterFrame.runner_kinds` (it does — locked in by the test above).
...
- This satisfies the helper's "Advertise only initialized runners" rule
  ...
  platform's PR 1 `[runner.openclaw]` TOML emission actually has an
  effect end-to-end.
