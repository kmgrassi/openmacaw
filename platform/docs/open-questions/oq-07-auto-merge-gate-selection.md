...
> the agent merges autonomously? `lint + tests + peer-review`? Just
...
`lint + tests + self-review + peer-review`, with a per-workspace
trust dial.
...
repo-declared gates, the peer-review design (including the
...
3. **`self-review`** — the authoring agent re-reads its own diff
   against the workspace review checklist. Cheap pre-filter.
4. **`peer-review`** — a *separate* agent (ideally on a different
   model) reviews the diff against the same checklist. See
   *[Peer-review: cross-LLM by default](#peer-review-cross-llm-by-default)*
   below.
...
create table auto_merge_required_gate (
  ...
  gate_kind           text not null check (gate_kind in (
                        'lint', 'tests', 'self-review', 'peer-review', 'custom'
                      )),
...
policy). See *[Peer-review: cross-LLM by default](#peer-review-cross-llm-by-default)*
for the design.
...
create table auto_merge_audit (
  ...
  gate_results        jsonb not null,                                           -- {lint: pass, tests: pass, peer-review: {verdict: approve, reviewer_model: ...}, ...}
...
|-------------|---------|-----------------------------------------------|----------|
| Conservative| false   | lint + tests + peer-review + self-review      | 100      |
| Balanced    | true    | lint + tests + peer-review + self-review      | 200      |
| Aggressive  | true    | lint + tests + self-review                    | 500      |
...
peer-review is a hard veto: even if every other gate is green, a
...
`prompts/peer-review-checklist-v1.md`, versioned the same way as
...
- Drop to the `Aggressive` preset (no peer-review). Strongly
...
4. **Gate runner.** Behavior callback per gate kind (`lint`,
   `tests`, `self-review`, `peer-review`, `custom` — backed by
...
9. **Reviewer prompt versioning.** Land
   `prompts/peer-review-checklist-v1.md` and
   `prompts/reviewer-system-v1.md`. (one PR in
   `parallel-agent-runtime`)
10. **Cost telemetry.** Per-PR cost split (author tokens vs
    reviewer tokens) so users can see what peer-review actually
...
- Should peer-review have a "veto" capability (block merge even if
  other gates are green)? Recommendation: yes — a peer-review fail
