...
API_CMD="(cd \"$ROOT_DIR/apps/api\" && \
  ...
  HOST=127.0.0.1 \
  PORT=$API_PORT \
  ORCHESTRATOR_BASE_URL=\"\${OPENMACAW_ORCHESTRATOR_BASE_URL:-\${ORCHESTRATOR_BASE_URL:-http://127.0.0.1:4000}}\" \
  ORCHESTRATOR_WS_URL=\"\${OPENMACAW_ORCHESTRATOR_WS_URL:-\${ORCHESTRATOR_WS_URL:-ws://127.0.0.1:4000}}\" \
  LOCAL_RELAY_DEV_TOKEN=\"\${LOCAL_RELAY_DEV_TOKEN:-lrh_dev_local_token_2026}\" \
  LAUNCHER_BASE_URL=\"\${OPENMACAW_LAUNCHER_BASE_URL:-\${LAUNCHER_BASE_URL:-http://127.0.0.1:4100}}\" \
